	...
	public void stdInsert(StudentVO vo) {
		...
		try {
			...
			String sql = "INSERT INTO student(hakbun,name,kor,eng,math) "
					...
					+ "?,?,?,?)";
			// SQL Injection
			// PreparedStatement => 미리 SQL 전송 => 나중에 값을 채워서 실행
			...
			// 실행 요청
			ps.executeUpdate(); //insert / update / delete 			=> commit이 자동으로 들어감
			// ps.executeQuery(); //select							=> commit이 없음
