-- 데이터 조작언어 (DML => INSERT / UPDATE / DELETE) = 187 page
/*
   INSERT : 데이터 추가
      = INSERT ALL : 이미 존재하는 테이블에서 전체데이터를 나눠서 저장
      *** = 전체 데이터 등록
...
-- INSERT INTO student VALUES(101,'심청이',90,80,95,SYSDATE);
/* 
INSERT INTO student(hakbun, name, kor, eng, math)
VALUES(102, '박문수', 78, 89, 90);
commit;
 */
-- subquery => insert, update, delete에서 사용이 가능
-- insert => 자동증가번호 만들기 SELECT NVL(MAX(hakbun)+1,100) FROM 테이블명
/* 
INSERT INTO student(hakbun, name, kor, eng, math)
VALUES((SELECT NVL(MAX(hakbun)+1,1) FROM student),
	'이순신',89, 90, 98);
