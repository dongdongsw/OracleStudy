...
/*
DECLARE
	no NUMBER:= 1; -- C언어 형식 :변수를 반듯 먼저 선언
BEGIN
	WHILE no <= 10 LOOP
...
/*
	형식)
		...
		DECLARE
			변수 선언 : 스칼라변수 ( 일반 = 변수명 데이터형)
				%TYPE: 실제 테이	블의 컬럼 데이터형을 가지고 온다
				CURSOR : 목록 => ResultSet
		BEGIN		
			연산자 : 연산자는 그대로 사용
			제어문 : IF ~, IF ~ ELSE, FOR
