 -- 5장 (프로그램: 오라클 프로그램, JDBC프로그램, WEB 프로그램)
 /*
   ...
   문법
      1. 변수
         ...
         = rowtype 변수
            전체 column의 데이터형 읽기
            emp emp%ROWTYPE
                ----- emp가 가지고 있는 모든 컬럼 = VO (자바)
         = record
            테이블 여러개에서 데이터 변수 : JOIN
      ---------------------- (스칼라, type, rowtype, record) 단점 : ROW단위
         = ***cursor
            여러개의 ROW를 동시에 처리
            => ArrayList => ResultSet
         => 라이브러리 제작 / 메뉴얼
 ...
 */
 ...
 /*
 -- 변수
 -- System.out.println() => DBMS_OUTPUT.PUTLINE()
 -- Scanner => &
 ...
 */
 /*
 -- ROWTYPE => VO : 한개의 테이블이 가지고 있는 모든 데이터형을 읽어 온다
 -- 테이블변수 테이블명%ROWTYPE
 DECLARE
   pEmp emp%ROWTYPE;
 BEGIN
...
-- 스칼라 변수 (변수 데이터형), TYPE 변수 (실제 데이터형 읽기)
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('이름:'||pename);
	DBMS_OUTPUT.PUT_LINE('직위:'||pjob);
	DBMS_OUTPUT.PUT_LINE('부서명:'||pDname);
...
BEGIN 
	...
	-- 				연산수행이 안된다
	IF pComm > 0 THEN
		DBMS_OUTPUT.PUT_LINE(pEname||'님의 급여는 '||pSal||'이고 성과급은
		'||pComm||'이며 총급여는 '||pTotal||'입니다');
	ELSE
		DBMS_OUTPUT.PUT_LINE(pEname||'님의 급여는 '||pSal||'이고 성과급은 없고
		총급여는 '||pTotal||'입니다');
	END IF;
END;
/
