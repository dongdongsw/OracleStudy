...
/*
	트리거 : 특정 이벤트 (INSERT / UPDATE / DELETE) 가 발생했을 경우에 자동저장 프로시저
	특징
		테이블 / 뷰에 연결
		이벤트가 발생하면 자동 호출
	...
	형식 
		CREATE [OR REPLACE] TRIGGERR tri_name
		...
		-------------------------------------------------------------------------
		시점 : 언제 트리거 처리
		BEFORE : DML 실행전에 동작 => 발생한 테이블ㄹ 저장에 trigger수행
		AFTER : 의 실행후에 동작	=> 발생한 테이블 저장후에 trigger 수행
...
CREATE SEQUENCE el_id_seq
	...
	NOCACHE;
*/
-- 트리거는 자동으로  COMMIT => COMMIT 사용하면 오류 발생
/*
...
CREATE OF REPLACE TRIGGER tri_backup
BEFORE DELETE ON emp_trg
FOR EACH ROW
BEGIN
	INSERT INTO emp_backupu(empno,ename,sal,deptno)
