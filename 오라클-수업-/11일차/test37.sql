...
/*
주의점
	트리거 안에서 DML 실행시 참조시에 무한루프
	복잡한 SQL 문장은 트리거보다 프로시저를 권장
	트리거는 디버깅이 어렵다	(테스트를 많이 해야한다)
	
	=> 키워드
		INSERTING / UPDATING / DLETEING
								| DELETE 문장이 실행중
					| UPDATE 문장이 실행중
		| INSERT 문장이 실행중
		=> BOOLEAN
...
BEGIN
	IF INSERTING THEN
		DBMS_OUTPUT.PUT_LINE('INSERT 발생:'||:NEW.empno);
	ELSIF UPDATING THEN
		DBMS_OUTPUT.PUT_LINE('UPDATE 발생:'||:OLD.empno||''||:NEW.empno);
	ELSIF DELETING THEN
		DBMS_OUTPUT.PUT_LINE('DELETE 발생:'||:OLD.empno);
	END IF;
END;
/
*/
...
INSERT INTO 상품 VALUES(100,'감자깡',1500);
INSERT INTO 상품 VALUES(100,'맛동산',2000);
INSERT INTO 상품 VALUES(100,'짱구',500);
INSERT INTO 상품 VALUES(100,'고구마깡',2500);
COMMIT;
*/
