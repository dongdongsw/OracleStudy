...
/*
	...
	JOIN  : 여러개의 테이블을 연결 필요한 데이터를 추출
			-------------- 정규화 => 테이블이 많이 나눠진다
			=> 데이터 추출
	SUBQUERY : 여러개의 SQL 한개의 SQL문장으로 변경
			   SELECT 뒤에  => 스칼라 서브쿼리(컬럼 대신)
			   FROM 뒤에 => 인라인뷰(테이블 대신)
			   WHERE 뒤에 => 서브쿼리(조건값 대신)
	...
	1. GROUP BY => 159page
		...
		2) 실행 순서
			...
			=> 컬럼 값중에 같은 값을 가지고 있는 것을 찾아서 그룹
			예) emp
				deptno(부서 번호) / job(직위) / 입사 년도별(hiredate)
			=> SQL 문장
				GROUP BY 컬럼 / 함수
				SELECT 컬럼명 | 함수명 , 집계함수...
				FROM table_name
				GROUP BY 컬럼명 / 함수명
				HAVING 조건 (그룹에 대한 조건) => GROUP BY있는 경우
				사용이 가능(필요시에만 사용)
				ORDER BY 컬럼명 | 함수명
...
ORDER BY TO_CHAR(hiredate,'DY"요일"');
*/
...
/*
SELECT
TO_CHAR(hiredate,'YYYY'),TO_CHAR(hiredate,'dy'),COUNT(*),SUM(sal),AVG(sal)
FROM emp
