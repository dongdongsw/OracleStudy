			...
			for(int i = 1; i <= 346; i++ ) {
				...
				for(int j=0; j < link.size(); j++) {
					try {
						...
						Elements name = doc2.select("div.areaBasic dd.name");
						...
						Elements type = doc2.select("div.areaBasic dd.type");
						System.out.println("업종 : " + type.text());
						
						Elements phone = doc2.select("div.areaBasic dd.tel1");
						System.out.println("전화 : " + phone.text());
						
						Elements address = doc2.select("div.areaBasic dd.add1");
						System.out.println("주소 : " + address.text());

						Elements score = doc2.select("div.areaBasic p.score span.total");
						System.out.println("평점 : " + score.text());
						
						Elements theme = doc2.select("div.areaBasic dd.theme");
						System.out.println("테마 : " + theme.text());

						Elements price = doc2.select("div.areaBasic p.price");
						System.out.println("가격 : " + price.text());
						
						Elements time = doc2.select("div.infoTable ul.tableTopA dd.txt2");
						System.out.println("영업시간 :"  + time.text());
						
						Elements content= doc2.select("div.infoTable ul.tableBottom div#info_ps_f");
						...
						System.out.println("메인페이지" + poster.attr("src"));
